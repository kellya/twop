...
def _init_config():
    ...
    for childProject in childProjects:
        config['maps'].append( { childProject['identifier'] : ''} )
    ...
    print()
    sys.exit('Add information about TaskWarrior projects in your twop.json configuration file')
...
def main():
    ...
    op = openproject.openproject(config['op']['baseUrl'],config['op']['apiKey'])
    opWPs = op.searchWorkPackage(config['op']['userId'],config['lessThanDaysAgo'],config['op']['projectID'])

    for wp in opWPs:
        wpf = op.directCall(wp['_links']['self']['href'])
        op._ppjson(wpf)
